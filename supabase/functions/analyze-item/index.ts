...
async function cloudVisionJson(params: {
  apiKey: string;
  mimeType: string;
  base64Data: string;
}) {
  const { apiKey, base64Data } = params;
  ...
  if (!res.ok) {
    const errText = await res.text().catch(() => "");
    throw new Error(`Cloud Vision error ${res.status}: ${errText}`);
  }
  ...
  return {
    nombre_corto: objects[0] || labels[0] || "Objeto desconocido",
    categoria: labels[0] || "Otros",
    descripcion: `Detectado por Cloud Vision: ${labels.slice(0, 5).join(", ")}`,
    color: null,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const CLOUD_VISION_API_KEY = Deno.env.get("CLOUD_VISION_API_KEY") ?? GEMINI_API_KEY;
    ...
    for (let i = 0; i < providers.length; i++) {
      ...
      try {
        ...
        if (p === "gemini") {
          ...
          if (!CLOUD_VISION_API_KEY) throw new Error("Cloud Vision error 401: Missing CLOUD_VISION_API_KEY");
          json = await cloudVisionJson({
