...
class ObjectRecognitionService {
  ...
  private async analyzeWithEdgeFunctions(imageDataUrl: string): Promise<RecognizedObject> {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/gemini-analyze`, {
        method: 'POST',
        headers: {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/groq-analyze`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY}`
