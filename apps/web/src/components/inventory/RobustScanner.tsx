...
export function RobustScanner({ onItemAdded, onScanSuccess, onClose }: ScannerProps) {
  ...
  const saveItem = async (result: any) => {
    ...
    try {
      ...
      const { containersService } = await import('@/core/containers');
      ...
      if (!containerId) {
         const allBoxes = await containersService.getAll();
         if (allBoxes && allBoxes.length > 0) {
             containerId = allBoxes[0].id;
         } else {
             const newBox = await containersService.create({
                 id: crypto.randomUUID(),
                 label: 'Caja General ' + new Date().getFullYear(),
                 qr_payload: 'GEN-' + Date.now(),
                 location_id: '00000000-0000-0000-0000-000000000000'
             });
             containerId = newBox.id;
         }
      }
